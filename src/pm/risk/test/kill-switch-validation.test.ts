...
describe("KillSwitch — KillLevel validation", () => {
  ...
  it("validateKillLevel throws for invalid string", () => {
    assert.throws(
      () => validateKillLevel("INVALID"),
      /KillSwitch: invalid kill level/,
    );
    assert.throws(
      () => validateKillLevel(""),
      /KillSwitch: invalid kill level/,
  ...
  it("validateKillLevel throws for non-string", () => {
    assert.throws(
      () => validateKillLevel(null as unknown),
      /KillSwitch: invalid kill level/,
    );
    assert.throws(
      () => validateKillLevel(123 as unknown),
      /KillSwitch: invalid kill level/,
    );
    assert.throws(
      () => validateKillLevel({} as unknown),
      /KillSwitch: invalid kill level/,
    );
  });
  ...
  it("levelAtLeast validates both arguments", () => {
    assert.throws(
      () => levelAtLeast("INVALID" as KillLevel, "NONE"),
      /KillSwitch: invalid kill level/,
    );
    assert.throws(
      () => levelAtLeast("NONE", "BAD" as KillLevel),
      /KillSwitch: invalid kill level/,
    );
  });
