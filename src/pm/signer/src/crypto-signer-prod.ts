...
function loadKeypair(privateKeyHex: string): KeyPair {
  if (!privateKeyHex || privateKeyHex.length !== 64) {
    throw new Error("INVALID_PRIVATE_KEY: Must be 64 hex characters (32 bytes)");
  }
...
function signDeterministic(keyPair: KeyPair, messageHash: Buffer): string {
  ...
  const signature = keyPairEC.sign(messageHash, { canonical: true, k: undefined });
 ...
 */
function verifySignature(publicKey: Buffer, messageHash: Buffer, signatureHex: string): boolean {
  try {
 ...
 */
export function createProductionCryptoSigner(config: ProdSignerConfig): CryptoSigner {
  const { privateKeyHex, chainId = 137 } = config;
  
  if (!privateKeyHex) {
    throw new Error("PRODUCTION_SIGNER_ERROR: PRIVATE_KEY_HEX environment variable required");
  }
  ...
  return async (request: SignRequest): Promise<string> => {
    ...
    if (request.payloadHash !== expectedHash) {
      throw new Error(`PAYLOAD_HASH_MISMATCH: Expected ${expectedHash}, got ${request.payloadHash}`);
    }
    
    // 2. Verify chain ID matches
    if (request.expectedChainId !== chainId) {
      throw new Error(`CHAIN_ID_MISMATCH: Expected ${chainId}, got ${request.expectedChainId}`);
    }
    
    // 3. Verify payload hash is valid hex (32 bytes = 64 hex chars)
    const payloadHashBytes = Buffer.from(request.payloadHash.replace("0x", ""), "hex");
    if (payloadHashBytes.length !== 32) {
      throw new Error(`INVALID_PAYLOAD_HASH_LENGTH: Expected 32 bytes, got ${payloadHashBytes.length}`);
    }
    ...
    if (!isValid) {
      throw new Error("SIGNATURE_VERIFICATION_FAILED: Generated signature failed verification");
    }
...
export function createSignerFromEnv(chainId: number = 137): CryptoSigner {
  ...
  const privateKeyHex = env["PRIVATE_KEY_HEX"] ?? env["WALLET_PRIVATE_KEY"]?.replace("0x", "");
  
  if (!privateKeyHex) {
    throw new Error("PRIVATE_KEY_HEX or WALLET_PRIVATE_KEY environment variable required for production signer");
  }
 ...
 */
export function createSignerFromHex(privateKeyHex: string, chainId: number = 137): CryptoSigner {
  return createProductionCryptoSigner({ privateKeyHex, chainId });
}
