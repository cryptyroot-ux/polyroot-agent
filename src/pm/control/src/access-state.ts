...
export type AccessAction =
  "ORDER_SUBMIT" | "ORDER_CANCEL" | "REDUCE_ONLY" | "READ";
    ...
    case "ACCESS_BLOCKED":
      return action === "READ"
        ...
        : {
            ok: false,
            code: "ACCESS_BLOCKED",
            reason: `compliance state ${state}: only read allowed`,
          };
    case "CLOSE_ONLY":
      if (action === "ORDER_CANCEL" || action === "REDUCE_ONLY") {
        return {
          ok: true,
          reason: "close_only: platform-permitted cancel/reduce only",
        };
      }
      return {
        ok: false,
        code: "ACCESS_CLOSE_ONLY",
        reason: "close_only: no new entries",
      };
    case "UNKNOWN":
    default:
      return {
        ok: false,
        code: "ACCESS_UNKNOWN",
        reason: "unknown access state — fail closed",
      };
  }
}
