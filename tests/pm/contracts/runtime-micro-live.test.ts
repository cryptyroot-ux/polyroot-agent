...
describe("Runtime MICRO_LIVE — G4 pipeline with real wallet/venue adapter and explicit capital cap", () => {
  ...
  function makeMicroLivePipeline(forecastP: number | null, customAdapter?: VenueAdapter, customCapUsd?: number, sizeIntentVal = 10) {
    ...
    return { pipeline, balance, venueAdapter: adapter as MicroLiveVenueAdapter };
  }
  ...
  it("tradable market in MICRO_LIVE produces decision + real venue submission (respects cap)", async () => {
    const { pipeline, venueAdapter, balance } = makeMicroLivePipeline(0.65, undefined, 500, 10);
    const result = await pipeline.processMarket({ market_id: "mkt_microlive_1", bid: 0.45, ask: 0.55 });
    ...
    assert.equal(venueAdapter.placeOrderCalls, 1, "MICRO_LIVE mode should call placeOrder on venue adapter");
    assert.equal(balance.reserveCalled, true, "MICRO_LIVE mode should call reserveFunds for reservation");
  });
  ...
  it("uncertain forecast in MICRO_LIVE abstains with NO_TRADE (no venue submission)", async () => {
    ...
    const result = await pipeline.processMarket({ market_id: "mkt_microlive_2", bid: 0.45, ask: 0.55 });
  ...
  it("MICRO_LIVE respects explicit capital cap - refuses order exceeding cap", async () => {
    const { pipeline, venueAdapter, balance } = makeMicroLivePipeline(0.65, undefined, 10, 100);
    ...
    assert.equal(result.decision, "NO_TRADE");
    assert.ok(result.reason?.includes("exceeds max_order_pct") || result.reason?.includes("exceeds"));
  ...
  it("MICRO_LIVE respects ENTRY_BLOCKED when venue is unavailable", async () => {
    ...
    const { pipeline, venueAdapter, balance } = makeMicroLivePipeline(0.65, unavailableAdapter);
    const result = await pipeline.processMarket({ market_id: "mkt_microlive_4", bid: 0.45, ask: 0.55 });
