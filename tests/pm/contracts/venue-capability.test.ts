...
describe("PR-EXE-02 / T-PR-EXE-02: VenueAdapter capability contract", () => {
  ...
  it("fails closed when a required capability is unsupported", () => {
    const res = capabilityAllows("order_submit", {
      venues: ["polymarket"],
      requires: ["order_submit"],
      supports: new Map([["order_submit", true]]),
    });
    assert.equal(res.ok, true);
    const blocked = capabilityAllows("order_submit", {
      venues: ["polymarket"],
      requires: ["order_submit"],
      supports: new Map([["order_submit", false]]),
    });
    assert.equal(blocked.ok, false);
  });

  it("an unregistered capability is refused (fail closed)", () => {
    const res = capabilityAllows("bridge_withdraw", {
      venues: ["polymarket"],
      requires: ["bridge_withdraw"],
      supports: new Map(),
    });
