...
describe("PR-GOV-03/05: Autonomy Charter is immutable and binds hard authority", () => {
  ...
  it("commission creates a charter that allows eligible routine intents (zero per-trade approval)", () => {
    ...
    const res = charterAllows(c, {
      action: "SUBMIT",
      strategyId: "evidence_directional_v2@1.0.0",
      exposureUsd: 50,
    });
  ...
  it("rejects promotion / capital-increase / extend actions (owner governance only)", () => {
    ...
    assert.equal(
      charterAllows(c, { action: "INCREASE_CAPITAL", exposureUsd: 5000 }).ok,
      false,
    );
    assert.equal(
      charterAllows(c, { action: "EXTEND_CHARTER", exposureUsd: 0 }).ok,
      false,
    );
    assert.equal(
      charterAllows(c, { action: "CHANGE_SIGNER", exposureUsd: 0 }).ok,
      false,
    );
  });
