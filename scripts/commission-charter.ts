...
program
  ...
  .requiredOption(
    "--capital-usd-cap <number>",
    "Maximum capital in USD (hard cap)",
  )
  .requiredOption(
    "--daily-loss-stop-pct <number>",
    "Daily loss stop percentage (0.05 = 5%)",
  )
  .requiredOption(
    "--strategy-ids <ids...>",
    "Qualified strategy IDs (space-separated)",
  )
  .requiredOption(
    "--market-classes <classes...>",
    "Allowed market classes (space-separated)",
  )
  .option("--expires-days <days>", "Charter validity in days", "365")
  .requiredOption(
    "--release-ref <ref>",
    "Release reference (e.g., v0.1.0-stable)",
  )
  ...
  .action((options) => {
    try {
      ...
      expiresAt.setDate(
        expiresAt.getDate() + parseInt(options.expiresDays, 10),
      );
      ...
      console.log(
        `   Qualified Strategies: ${charter.qualified_strategy_ids.join(", ")}`,
      );
      console.log(
        `   Market Classes: ${charter.market_class_allowlist.join(", ")}`,
      );
      ...
      console.log(
        "   1. Store this file securely (required for LIVE mode startup)",
      );
      console.log(
        "   2. Reference in process manager config or mount as volume",
      );
